...
export function getSuccesRepeatsPercent(state, i){
    let count = 0;
    let all = 0;
    state.learning.lastRepeats[i].forEach(el => {
        count+=el;
        all++;
    }) 
    let res = '-';
    res =  all === 0 ? res : (count*100/all).toFixed(0)+'%';
    return res;
}


export function getIntervalTime(state, i){
    console.log(state)
    const interval = state.learning.coefficients[i]*state.learning.intervals[i];
    let timeLearning = 0;
    if (interval >= 2592000000){
        timeLearning = Math.round(interval / 2592000000) + ' мес.'
    } else if (interval >= 604800000){
        timeLearning = Math.round(interval / 604800000) + ' нед.'
    } else if (interval >= 86400000){
        timeLearning = Math.round(interval / 86400000) + ' д.'
    } else if (interval >= 3600000) {
        timeLearning = Math.round(interval / 3600000) + ' ч.'
    } else if (interval >= 60000){
        timeLearning = Math.round(interval / 60000) + ' мин.'
    } else {
        timeLearning = Math.round(interval / 1000) + ' сек.'
    }
    return timeLearning
}
...
export function getRepeatTimeEnding(state){
    ...
    for (let i=0;i<state.intervals.length;i++){
        state.repeat[i].forEach(word => {
            interval = Math.min(interval, state.intervals[i]*state.coefficients[i] - Date.now() + word.time)     
        })
    }
...
export function getRepeatPhrase(state, setRepeatTime){
    let wordsCount = 0
    for (let i = 0; i < state.intervals.length; i++) {
        state.repeat[i].forEach(el => {
            if (Date.now() - state.intervals[i]*state.coefficients[i] > el.time) {
                wordsCount++
            } else {
                setRepeatTime(
